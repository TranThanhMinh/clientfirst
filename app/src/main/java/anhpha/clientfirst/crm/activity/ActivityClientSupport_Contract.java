...
public class ActivityClientSupport_Contract extends BaseAppCompatActivity implements adapter_client_support.Click{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("AAA", mOrder.getClient_id() + "");
        tvName.setText(mOrder.getOrder_contract_name());
    ...
    public void funcAddSupport(List<MId> list) {
        ServiceAPI api = retrofit.create(ServiceAPI.class);
        Call<Result> call = api.set_order_contract_detail_users(preferences.getStringValue(Constants.TOKEN, ""), preferences.getIntValue(Constants.USER_ID, 0), preferences.getIntValue(Constants.PARTNER_ID, 0), mOrder.getOrder_contract_id(), list);
        call.enqueue(new Callback<Result>() {
            ...
            public void onResponse(Call<Result> call, Response<Result> response) {
                ...
                if (response.body().getHasErrors() == false) {
                    ...
                    Toast.makeText(ActivityClientSupport_Contract.this, "Loi", Toast.LENGTH_SHORT).show();
                }
            }
    ...
    public void funcDeleteSupport(int id) {
        ServiceAPI api = retrofit.create(ServiceAPI.class);
        Call<Result> call = api.delete_order_contract_detail_user(preferences.getStringValue(Constants.TOKEN, ""), preferences.getIntValue(Constants.USER_ID, 0), preferences.getIntValue(Constants.PARTNER_ID, 0),mOrder.getOrder_contract_id(), id);
        call.enqueue(new Callback<Result>() {
            @Override
            public void onResponse(Call<Result> call, Response<Result> response) {
                LogUtils.api("", call, response);
    ...
    public void getClientSupport() {
        ServiceAPI api = retrofit.create(ServiceAPI.class);
        Call<MAPIResponse<List<MUser>>> call = api.get_order_contract_detail_users(preferences.getStringValue(Constants.TOKEN, ""), preferences.getIntValue(Constants.USER_ID, 0), preferences.getIntValue(Constants.PARTNER_ID, 0), mOrder.getOrder_contract_id());
        call.enqueue(new Callback<MAPIResponse<List<MUser>>>() {
            ...
            public void onResponse(Call<MAPIResponse<List<MUser>>> call, Response<MAPIResponse<List<MUser>>> response) {
                TokenUtils.checkToken(ActivityClientSupport_Contract.this, response.body().getErrors());
                LogUtils.api("", call, response);
                box.hideAll();
                list = response.body().getResult();
                if (list != null) {
                    adapter_client_support adapter = new adapter_client_support(ActivityClientSupport_Contract.this, list,ActivityClientSupport_Contract.this);
