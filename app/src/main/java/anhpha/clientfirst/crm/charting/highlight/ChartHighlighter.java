...
import anhpha.clientfirst.crm.charting.interfaces.dataprovider.BarLineScatterCandleBubbleDataProvider;
 ...
 */
public class ChartHighlighter<T extends BarLineScatterCandleBubbleDataProvider> implements anhpha.clientfirst.crm.charting.highlight.IHighlighter
{
    ...
    public anhpha.clientfirst.crm.charting.highlight.Highlight getHighlight(float x, float y) {
        ...
        anhpha.clientfirst.crm.charting.highlight.Highlight high = getHighlightForX(xVal, x, y);
        return high;
    }
     ...
     * @return
     */
    protected anhpha.clientfirst.crm.charting.highlight.Highlight getHighlightForX(float xVal, float x, float y) {

        List<anhpha.clientfirst.crm.charting.highlight.Highlight> closestValues = getHighlightsAtXValue(xVal, x, y);
        ...
        anhpha.clientfirst.crm.charting.highlight.Highlight detail = getClosestHighlightByPixel(closestValues, x, y, axis, mChart.getMaxHighlightDistance());
     ...
     * @return
     */
    protected float getMinimumDistance(List<anhpha.clientfirst.crm.charting.highlight.Highlight> closestValues, float pos, YAxis.AxisDependency axis) {

        float distance = Float.MAX_VALUE;

        for (int i = 0; i < closestValues.size(); i++) {

            anhpha.clientfirst.crm.charting.highlight.Highlight high = closestValues.get(i);
    ...
    protected float getHighlightPos(anhpha.clientfirst.crm.charting.highlight.Highlight h) {
        return h.getYPx();
    }
     ...
     * @return
     */
    protected List<anhpha.clientfirst.crm.charting.highlight.Highlight> getHighlightsAtXValue(float xVal, float x, float y) {
    ...
    protected List<anhpha.clientfirst.crm.charting.highlight.Highlight> buildHighlights(IDataSet set, int dataSetIndex, float xVal, DataSet.Rounding rounding) {
        ...
        for (Entry e : entries) {
            ...
            highlights.add(new anhpha.clientfirst.crm.charting.highlight.Highlight(
                    e.getX(), e.getY(),
     ...
     * @return
     */
    public anhpha.clientfirst.crm.charting.highlight.Highlight getClosestHighlightByPixel(List<anhpha.clientfirst.crm.charting.highlight.Highlight> closestValues, float x, float y,
                                                                                          YAxis.AxisDependency axis, float minSelectionDistance) {

        anhpha.clientfirst.crm.charting.highlight.Highlight closest = null;
