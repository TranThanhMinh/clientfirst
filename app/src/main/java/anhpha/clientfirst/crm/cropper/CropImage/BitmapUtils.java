...
final class BitmapUtils {
     ...
     */
    static Bitmap resizeBitmap(Bitmap bitmap, int reqWidth, int reqHeight, CropImage_View.RequestSizeOptions options) {
        try {
            if (reqWidth > 0 && reqHeight > 0 && (options == CropImage_View.RequestSizeOptions.RESIZE_FIT ||
                    options == CropImage_View.RequestSizeOptions.RESIZE_INSIDE ||
                    options == CropImage_View.RequestSizeOptions.RESIZE_EXACT)) {

                Bitmap resized = null;
                if (options == CropImage_View.RequestSizeOptions.RESIZE_EXACT) {
                    resized = Bitmap.createScaledBitmap(bitmap, reqWidth, reqHeight, false);
                } else {
                    int width = bitmap.getWidth();
                    int height = bitmap.getHeight();
                    float scale = Math.max(width / (float) reqWidth, height / (float) reqHeight);
                    if (scale > 1 || options == CropImage_View.RequestSizeOptions.RESIZE_FIT) {
